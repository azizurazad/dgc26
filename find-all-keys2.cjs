        ...
        if (ts.isJsxElement(body) || ts.isJsxSelfClosingElement(body)) {
          ...
          if (!hasKey) {
            ...
            console.log(`${file.fileName}: Missing key at line ${pos.line + 1} for element <${tag.tagName.getText()}>`);
             ...
             const keyProp = tag.attributes.properties.find(p => p.name && p.name.text === 'key');
             console.log(`${file.fileName}: Found key at line ${file.getLineAndCharacterOfPosition(body.getStart()).line + 1}: ${keyProp.initializer.getText()}`);
          }
            ...
            if (ts.isJsxElement(expr) || ts.isJsxSelfClosingElement(expr)) {
              ...
              if (!hasKey) {
                ...
                console.log(`${file.fileName}: Missing key at line ${pos.line + 1} for element <${tag.tagName.getText()}>`);
                 ...
                 const keyProp = tag.attributes.properties.find(p => p.name && p.name.text === 'key');
                 console.log(`${file.fileName}: Found key at line ${file.getLineAndCharacterOfPosition(expr.getStart()).line + 1}: ${keyProp.initializer.getText()}`);
              }
